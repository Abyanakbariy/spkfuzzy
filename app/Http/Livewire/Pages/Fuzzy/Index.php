...
class Index extends Component
{
    ...
    public function submit()
    {
        ...
        $this->inggridient = InggridientHistory::select('inggridient_history.date', 'inggridient_history.id_history', 'inggridient_history.id_inggridient','master_inggridients.name_inggridient')
            ->join('master_inggridients','master_inggridients.id_inggridient','inggridient_history.id_inggridient')
            ->whereBetween('inggridient_history.date', [$start, $end])
            ->groupBy('inggridient_history.id_inggridient')
            ->get();
        ...
        foreach ($this->inggridient as $key => $value) {
            ...
            $this->range[$key]['defuzzy'] = $this->range[$key]['nilai_atas'] == 0 ? 0 : ($this->range[$key]['nilai_atas'] / $this->range[$key]['nilai_keanggotaan']);
        }
